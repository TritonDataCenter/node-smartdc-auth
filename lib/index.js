...
function requestSigner(options) {
    ...
    if (options.mantaSubUser && options.subuser !== undefined)
        user += '/' + options.subuser;
    /*
     * CloudAPI separates user and subuser by one path component. It only
     * checks that there is *something* there, not what it actually is.
     * Existing SDC clients are using "users", so we'll use that too.
     */
    else if (options.subuser !== undefined)
        user += '/users/' + options.subuser;
