    ...
    fs.readdir(p, function (err, files) {
        ...
        function readPrivateKey(keyFile, kcb) {
            ...
            fs.readFile(fname, 'ascii', function (kerr, blob) {
                ...
                try {
                    var key = sshpk.parseKey(blob, 'pem', fname);
                } catch (e) {
                    kcb(e);
                    return;
                }
    ...
    client.requestIdentities(function (err, keys) {
        ...
        if (err) {
            ...
            for (var i = 0; i < keys.length; ++i) {
                var k = keys[i];
                var kp = sshpk.parseKey(k._raw, 'rfc4253',
                    'ssh-agent:' + k.comment);
                kp.comment = k.comment;
                kp._raw = k._raw;
...
function sshAgentSign(client, key, data, alg, cb) {
    ...
    var algType = alg.split('-')[0].toLowerCase();
    assert.strictEqual(algType, key.type,
        'key type ' + key.type + ' does not match signature algorithm ' + alg);
    ...
    client.sign(key, data, function (err, signature) {
        ...
        if (err) {
            ...
            sig = sshpk.parseSignature(sig, key.type, 'ssh');
            ...
            funcs: [
                ...
                function agentSign(opts, cb) {
                    if (!initOpts.agent || !initOpts.key ||
                        typeof (initOpts.key._raw) !== 'object')
                    {
                        cb();
                        return;
                    }
                ...
                function signWithPrivateKey(opts, cb) {
                    ...
                    var s = crypto.createSign(a.
                        replace(/^ecdsa/, 'ecdsa-with'));
                    s.update(str);
                    var sig = s.sign(k.privatePem, 'base64');
                    opts.res = {
